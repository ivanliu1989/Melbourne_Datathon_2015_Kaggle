...
#                     )
    ...
    write.csv(as.data.frame(pred),
              file=paste0('ReadyForBlending/validation/2_glm_d0', d0,
                          '_d1', d1, '_d2', d2, '_d3', d3, '_score', as.numeric(perf_new),'.csv'),quote = FALSE,row.names = FALSE)
    # write.csv(as.data.frame(pred),file=paste0('ReadyForBlending/validation/1_dl_0.8059_0.7874.csv'),quote = FALSE,row.names = FALSE)
