...
test <- validation #train[train$EVENT_ID %in% c(101183757,101183885,101184013),]#validation
...
#   bagTestPredictions_nn = as.data.frame(h2o.predict(object = BagModel, newdata = test_df))
  ...
  dtrain <- xgb.DMatrix(as.matrix(cbind(train[baggedIndex,feat],rf_n=bagPredictions_rf[,1],rf_y=bagPredictions_rf[,2]
                                        ...
                                        #nn_n = bagPredictions_nn$X0, nn_y=bagPredictions_nn$X1
  ...
  dtest <- xgb.DMatrix(as.matrix(cbind(test[,feat],rf_n=bagTestPredictions_rf[,1],rf_y=bagTestPredictions_rf[,2]
                                       ...
                                       #nn_n = bagTestPredictions_nn$X0, nn_y=bagTestPredictions_nn$X1
                                       )),label = test$flag_class)
  # watchlist <- list(eval = dtest, train = dtrain)
  BagModel <- xgb.train(data = dtrain, max.depth = 6, eta = 0.02, nround = 1200, #watchlist = watchlist, 
                        colsample_bytree = 0.8, min_child_weight = 2, verbose = 0, 
